...
data class MenuOp(@IdRes val id: Int, val name: String, val initializer: () -> BaseFragment) : MultiItemEntity {
    ...
    override val itemType: Int = Expand_Type_Item
...
data class MenuOpHead(val name: String) : MultiItemEntity {
    override val itemType: Int = Expand_Type_Head
    var isExpanded: Boolean = false
    private val _subItems: MutableList<MenuOp> = mutableListOf()
    val subItems: List<MenuOp> get() = _subItems
    val allSubItem: List<MenuOp> get() = subItems

    fun addSubItem(item: MenuOp) {
        _subItems.add(item)
    }
