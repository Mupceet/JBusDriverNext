...
class GenrePagePresenterImpl(val url: String) : BasePresenterImpl<GenrePageContract.GenrePageView>(),
    GenrePageContract.GenrePagePresenter {
    ...
    val model: BaseModel<String, Document> = object : AbstractBaseModel<String, Document>({ url ->
        ...
        override fun requestFromCache(t: String) =
            Flowable.concat(CacheLoader.justLru(url).map { Jsoup.parse(it) }, requestFor(t)).firstOrError().toFlowable()
    }
