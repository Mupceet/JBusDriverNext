...
abstract class AbstractRefreshLoadMorePresenterImpl<V : BaseView.BaseListWithRefreshView, T> : BasePresenterImpl<V>(),
    ...
    override fun onLoadMore() {
        when {
            ...
            pageInfo.nextPage >= Math.max(
                pageInfo.activePage, pageInfo.referPages.lastOrNull() ?: 1
            ) -> {
    ...
    override fun loadData4Page(page: Int) {
        var curPage = pageInfo.copy(activePage = page, nextPage = page)
        val request = (if (page == 1) model.requestFromCache(page) else model.requestFor(page))
        request.map { doc ->
            parsePage(doc)?.let { curPage = it }
            ...
            return@Function when {
                (it is HttpException && it.code() == 404) -> {
                    ...
                    if (curPage.nextPage > 1 && curPage.activePage > 0) pageInfo =
                        pageInfo.copy(activePage = pageInfo.nextPage - 1)
    ...
    private fun parsePage(pageDoc: Document): PageInfo? {
        with(pageDoc) {
            ...
            val pages = select(".pagination a:not([id])")
                .mapNotNull { it.attr("href").split("/").lastOrNull()?.toIntOrNull() }
            return PageInfo(
                current.split("/").lastOrNull()?.toIntOrNull() ?: 0,
                next.split("/").lastOrNull()?.toIntOrNull() ?: 0,
                pages
            ).apply {
                if (pages.isNotEmpty()) lastPage = pages.last()
            }
        }
    }
    ...
    open inner class ListDefaultSubscriber(private val currentPage: PageInfo) : SimpleSubscriber<ResultPageBean<T>>() {
        ...
        override fun onComplete() {
            super.onComplete()
            if (!hasLoadNext()) {
                if (currentPage.activePage == lastPage) mView?.loadMoreEnd(false)
                else mView?.loadMoreEnd(true)
            } else {
        ...
        override fun onNext(t: ResultPageBean<T>) {
            super.onNext(t)
            if (currentPage.activePage == 1) mView?.resetList()
            pageInfo = t.pageInfo
            doAddData(t)
            mView?.dismissLoading()
        }
