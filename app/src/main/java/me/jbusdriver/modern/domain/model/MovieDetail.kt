...
/**
 * 修正影片详情中所有链接的 host，使其指向当前生效的服务器地址。
 *
 * 职责：当用户切换了服务器 URL 后，已缓存的影片详情中的链接仍指向旧 host，
 *       此函数将所有子项的链接统一替换为新 host，避免链接失效。
 *
 * 使用场景：从缓存加载影片详情后，若检测到 host 变化则调用此函数修正链接。
 *
 * @param host 当前生效的服务器 host（如 "https://www.example.com"）
 * @return 链接已修正的 [MovieDetail] 副本，若无需修正则返回原对象
 */
