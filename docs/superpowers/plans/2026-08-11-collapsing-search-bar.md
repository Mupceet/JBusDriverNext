...
> **Revision (2026-08-11):** The first implementation used Material3 `TopAppBarDefaults.enterAlwaysScrollBehavior()` but never set `TopAppBarState.heightOffsetLimit` (it defaults to `-Float.MAX_VALUE` until a `TopAppBar` sets it), so the behavior consumed every scroll delta: the list could not scroll and the bar could not expand following the finger. An intermediate observe-only version followed the list 1:1 but scrolled the list during the collapse. Final approach: keep `enterAlwaysScrollBehavior()` and set `heightOffsetLimit` to the bar's measured height inside `CollapsingSearchBar`, so the bar collapses first (consuming scroll) and only then does the list scroll; the bar slides up out of view. The task code below reflects the final approach.
...
**Architecture:** `MainScreen` owns one `CollapsingSearchBar` at the top of its content `Column`, driven by Material3 `TopAppBarDefaults.enterAlwaysScrollBehavior()`. The tab content is wrapped with `Modifier.nestedScroll(scrollBehavior.nestedScrollConnection)`; `CollapsingSearchBar` sets `TopAppBarState.heightOffsetLimit` to the bar's measured height so the behavior stops consuming once the bar is fully hidden. The bar collapses first (consuming scroll so the list does not move), then the list scrolls; the bar slides up out of view. Pull-to-refresh is safe because at the top the bar is fully expanded and `PullToRefreshBox` handles the pull. The bar keeps its current visibility across tab switches (no reset/expansion on tab change).
